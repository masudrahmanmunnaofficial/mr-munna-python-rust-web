...
Browser records currently use localStorage. Each browser has its own data; clearing browser storage removes entries. Use **Backup & settings → Prepare backup → Save backup file** often. Restore accepts that JSON file and replaces the current browser records. Backups are JSON. Offline calculated prayer times are available for selectable locations, alongside a manual online check. Large media attachments are not yet implemented in this web version. The backup contains the browser records and worksheets; it is JSON rather than ZIP. The app makes no server account or password claim; this GitHub Pages site and its source are public. Sexual Health and Offline Vault are absent.
